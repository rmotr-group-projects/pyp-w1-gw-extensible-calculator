...
def perform_operation(calc, operation, params):
    ...
    if not all(map(lambda x: isinstance(x, float) or isinstance(x, int), params)):
        raise InvalidParams('Given params are invalid.')
    
    try:
        
        result = calc['operations'][operation](*params)
        calc['history'].append((str(datetime.now()).split('.')[0], 
                                operation, params, result))
        return result
        
    except:
        raise InvalidOperation('Given params are invalid.')
